...
Add a **Final Blow** field immediately beside **Victim** whenever a killmail contains an attacker marked with the Final Blow. Render both fields inline and use the same participant formatting and zKillboard links: a linked alliance ticker, falling back to a linked corporation ticker, followed by the linked character name. When the Final Blow has no character, show its Ship Type without an identity link while retaining an available linked affiliation.

If no Final Blow is marked, omit the new field and preserve the existing full-width Victim layout. If malformed input marks more than one attacker, display the first in killmail order. Keep the change confined to embed presentation and verify it through the existing credential-free embed-building seam.
...
4. As a killfeed reader, I want Victim and Final Blow formatted consistently, so that I can scan participant identity without learning two display conventions.
5. As a killfeed reader, I want a Final Blow character name linked to that character's zKillboard page, so that I can inspect the character directly.
...
7. As a killfeed reader, I want alliance affiliation preferred over corporation affiliation when both exist, so that Final Blow follows the established Victim presentation.
...
10. As a killfeed reader, I want an NPC or structure Final Blow represented by its Ship Type when it has no character, so that non-character attackers are still identifiable.
...
13. As a killfeed reader, I want Victim to remain full-width when no Final Blow is present, so that malformed or incomplete input does not leave an isolated narrow field.
14. As an operator, I want a killmail with multiple attackers marked for the Final Blow to render deterministically using the first one, so that malformed upstream data does not block delivery.
15. As an operator, I want missing or duplicated Final Blow markers handled without errors, alerts, or new validation infrastructure, so that the RedisQ processing loop remains resilient.
...
18. As a maintainer, I want Victim and Final Blow to share one small participant formatter, so that their text and link conventions cannot drift independently.
...
- Select the first attacker in killmail order whose Final Blow marker is true.
- Resolve Final Blow identity through the same existing name, Ship Type, and ticker cache/ESI paths used by killmail presentation. No new metadata client or cache is added.
- A small shared participant formatter renders both Victim and Final Blow. It accepts already-resolved identity and affiliation details; it does not become a broad domain abstraction.
...
- When a Final Blow can be displayed, emit Victim and Final Blow as adjacent inline fields after the existing attacker summary.
- When no attacker is marked for the Final Blow, emit no Final Blow field and keep Victim non-inline as it is today.
...
- Add three focused tests:
  1. A normal player Final Blow renders the exact linked Victim and Final Blow values in adjacent fields, with both fields inline.
  2. A Final Blow without a character renders the linked affiliation followed by the unlinked Ship Type.
...
- Showing Final Blow ship, weapon, damage, security status, or combat statistics for character attackers.
...
- zKillboard uses “final blow” for the credited participant: https://zkillboard.com/information/faq/
