...
Presentation revision 4 makes the regional public-contract observation the minimum durable location for every final embed, including historical rerenders. It resolves the human-readable region name from the same loaded system catalog used by the main killfeed, falling back to the unauthenticated public region endpoint during live collection, and retains any more-specific structure or solar-system snapshot through terminal acceptance. Historical rendering reuses retained event, snapshot, cache, and system-catalog evidence without issuing enrichment requests; it rejects the bounded queue atomically rather than exposing a raw region ID when no name is available. Authenticated structure requests continue to use only the scoped resolver client.
