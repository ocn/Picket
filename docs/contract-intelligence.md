...
The regional public-contract observation is the minimum durable location for every final embed. The collector stores its region ID, caches the human-readable name from the unauthenticated public region endpoint, and retains any more-specific structure or solar-system snapshot through terminal acceptance. If the public name lookup is temporarily unavailable, the embed still renders a linked `Region <ID>` line; authenticated structure requests continue to use only the scoped resolver client.
