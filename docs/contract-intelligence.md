...
Presentation revision 3 makes the regional public-contract observation the minimum durable location for every final embed, including historical rerenders. The collector stores its region ID, caches the human-readable name from the unauthenticated public region endpoint, and retains any more-specific structure or solar-system snapshot through terminal acceptance. Historical rendering reuses only retained event, snapshot, and cache evidence; it does not issue enrichment requests. If the public name lookup is temporarily unavailable, the embed still renders a linked `Region <ID>` line; authenticated structure requests continue to use only the scoped resolver client.
