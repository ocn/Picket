...
Public contract discovery, manifests, lifecycle probes, issuer facts, and delivery always remain unauthenticated. Set `STRUCTURE_RESOLVER_ENABLED=true` only after granting one dedicated EVE character exactly `esi-universe.read_structures.v1`; also set `STRUCTURE_RESOLVER_CHARACTER_ID`, the secret-injected `STRUCTURE_RESOLVER_REFRESH_TOKEN`, and existing `EVE_CLIENT_ID`/`EVE_CLIENT_SECRET`. The resolver validates refreshed JWT signature, issuer, audience, expiry, character, and exact scope before sending its bearer token only to the authenticated structure endpoint. It writes expiring Access-Qualified Location Evidence; public NPC evidence still outranks it, followed by unexpired operator evidence. Successful structure facts and ETags are retained independently of expiry; a restarted resolver revalidates expired entries with `If-None-Match` and reuses a `304 Not Modified` representation only with a renewed expiry. Structure response rate, retry, and ESI error-limit boundaries use the collector's shared persisted admission state.

Set `STRUCTURE_RESOLVER_ENABLED=false` to roll back only authenticated enrichment. Missing, revoked, invalid, or denied authorization reduces location precision but never gates public collection, lifecycle analysis, or ordinary delivery. Denied structures are retried at most daily and parked after seven elapsed days; increment `STRUCTURE_RESOLVER_CREDENTIAL_REVISION` after rotation to reactivate them. A refresh token returned by SSO is retained only in process memory and is used for later refreshes in that process. Ticket 08 owns updating the runtime secret provider: if a process restarts before that update, the stale credential degrades only the resolver until the operator supplies the new secret and bumps the revision. Do not place refresh tokens in logs, command output, diagnostics, database rows, or source-controlled files.
