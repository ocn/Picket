...
### 2026-09-04T23:00Z — Historical samples for #corp-watch; noisy sandbox subscription removed
- Operator asked for real examples to judge the hostile-group watch. Eight `prepared` rows were written for `corp-watch` exactly as the collector renders them (same JSON shape, footer suffixed "historical sample, event of <date>"), from real ESI data: joins of Drunkunt Dead [FFAZ] → X.I.X (2026-08-13), Belt to Asteroid [B 2 A] → FL33T (08-06), Blank-Space [TAYLR] → Snuffed Out (08-03), Suddenly Spaceless [0SLS0] → B0SS (07-31); Gears of War Inc. [GOW13] left X.I.X (06-18); war #762456 letus gogogo [JDIT] vs B0SS declared (08-27) and finished (08-30); war #762254 Shadow Cartel vs Relentless Miners [RLMI] declared (05-11, open for allies). Sources: `GET /corporations/{id}/alliancehistory/` over the 124 current member corporations, `watchlist_wars`, `GET /corporations/{id}/`.
- `discordbot` restarted 23:00:39–41Z to drain them (start-up lines present; killfeed resumed from checkpoint 993771). All 8 sent once within 12 s (`attempt_count` 1, Discord message ids recorded); no failures; duplicate-key query empty for both delivery tables (unscheduled G3-style check: PASS).
- Sov sandbox: the `hostile-tz` subscription (defender = watchlist, tz shift) was removed from `#sandbox` at 19:5xZ because its `appeared`/T-minus posts for any X.I.X hub anywhere are the noise the operator flagged; `reachable-timers` stays. The two earlier `appeared` deliveries and their T-120m follow-ups were cascade-deleted with the subscription (their evidence is in the 18:25Z entry). Ticket 19 (never post an unresolved system) is in implementation; ticket 20 (embed clarity, reachability display rule, per-subscription stages) waits on the FC feedback round.
- **Result:** PASS (samples delivered once; dedup held across an unscheduled restart)
