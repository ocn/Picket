...
impl<'a> LocationEvidenceService<'a> {
    ...
    pub async fn current_public_npc(
        &self,
        location_id: i64,
        now: DateTime<Utc>,
    ) -> Result<Option<LocationEvidence>, sqlx::Error> {
        ...
        sqlx::query("SELECT id, location_id, evidence_class, structure_id, station_id, solar_system_id, region_id, provenance, actor, observed_at, expires_at, expired_at, superseded_at, superseded_by_actor, supersedes_id FROM location_evidence WHERE location_id = $1 AND evidence_class = 'public_npc' AND observed_at <= $2 AND superseded_at IS NULL AND expired_at IS NULL AND (expires_at IS NULL OR expires_at > $2) ORDER BY observed_at DESC, id DESC LIMIT 1")
            .bind(location_id)
            .bind(now)
            .fetch_optional(self.pool)
            .await?
    ...
    pub async fn last_verified_access_qualified(
        &self,
        location_id: i64,
        now: DateTime<Utc>,
    ) -> Result<Option<LocationEvidence>, sqlx::Error> {
        ...
        sqlx::query("SELECT id, location_id, evidence_class, structure_id, station_id, solar_system_id, region_id, provenance, actor, observed_at, expires_at, expired_at, superseded_at, superseded_by_actor, supersedes_id FROM location_evidence WHERE location_id = $1 AND evidence_class = 'access_qualified' AND observed_at <= $2 AND superseded_at IS NULL AND expired_at IS NULL AND expires_at IS NOT NULL AND expires_at <= $2 ORDER BY observed_at DESC, id DESC LIMIT 1")
            .bind(location_id)
            .bind(now)
            .fetch_optional(self.pool)
            .await?
