//! The sov campaign collector: one cycle polls `/sovereignty/campaigns/`,
//! updates Sov Baseline / ended-campaign state, evaluates subscriptions
//! against newly appeared campaigns, and drives the durable delivery path.
//!
...
//! collection, and a single Alert Stage (`appeared`) instead of the full
//! contract lifecycle.

use crate::esi_cache::{merge_cache_metadata, EsiError, EsiLimiterStore};
use crate::sov_feed::model::{
    PreparedSovDelivery, SovAlertStage, SovCampaign, SovDelivery, SovEmbedField,
...
impl SovCollector {
        ...
        if !alert_candidates.is_empty() {
            ...
            for campaign in &alert_candidates {
                for subscription in &subscriptions {
                    ...
                    {
                        let message = self.render_appeared_message(campaign).await;
    ...
    async fn render_appeared_message(&self, campaign: &SovCampaign) -> SovNotificationMessage {
        ...
        SovNotificationMessage {
            title,
            fields,
            footer: SovAlertStage::Appeared.footer_label().to_string(),
        }
    }
